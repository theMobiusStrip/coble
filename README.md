...
Run on every push (scripted models — deterministic, no API key). Same tasks run against a real model with `coble eval -m openai:gpt-5.5`.

<!-- the table below is generated by `coble eval --write` -->

| Suite | Passed |
| --- | --- |
| scripted (CI) | **16/16** |

See [evals/RESULTS.md](./evals/RESULTS.md) for the per-task table, and [`evals/tasks/`](./evals/tasks) for the task definitions.
